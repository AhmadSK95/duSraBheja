    ...
    *,
    ...
    for entry in entries:
        ...
        await store.create_classification(
            ...
            capture_intent=str(entry.get("capture_intent") or "reference"),
            intent_confidence=float(entry.get("intent_confidence") or 0.85),
            validation_status=str(entry.get("validation_status") or "validated"),
            quality_issues=list(entry.get("quality_issues") or []),
            eligible_for_boards=bool(entry.get("eligible_for_boards", True)),
            eligible_for_project_state=bool(entry.get("eligible_for_project_state", True)),
        ...
        await publish_story_entry(
            ...
            outcome=entry.get("outcome") or story_fields["outcome"],
            impact=entry.get("impact") or story_fields["impact"],
            open_question=entry.get("open_question") or story_fields["open_question"],
